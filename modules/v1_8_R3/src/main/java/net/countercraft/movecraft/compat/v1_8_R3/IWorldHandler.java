...
public class IWorldHandler extends WorldHandler {
    ...
    public void rotateCraft(@NotNull Craft craft, @NotNull MovecraftLocation originPoint, @NotNull Rotation rotation) {
        ...
        for(BlockPosition position : rotatedPositions.keySet()){
            blockData.put(position, rotate(rotation, nativeWorld.getType(position)));
        }
    ...
    public void setBlockFast(@NotNull Location location, @NotNull Rotation rotation, @NotNull Material material, byte data) {
        ...
        blockData = rotate(rotation, blockData);
    ...
    private IBlockData rotate(final Rotation rotation, IBlockData originalData) {
        if (!(originalData.getBlock() instanceof BlockDirectional)) {
            return originalData;
        }

        EnumDirection eDir = originalData.get(BlockDirectional.FACING);
        EnumDirection rotated;
        switch (eDir) {
            case EAST:
                rotated = rotation == Rotation.CLOCKWISE ? EnumDirection.SOUTH : EnumDirection.NORTH;
                break;
            case WEST:
                rotated = rotation == Rotation.CLOCKWISE ? EnumDirection.NORTH : EnumDirection.SOUTH;
                break;
            case SOUTH:
                rotated = rotation == Rotation.CLOCKWISE ? EnumDirection.WEST : EnumDirection.EAST;
                break;
            case NORTH:
                rotated = rotation == Rotation.CLOCKWISE ? EnumDirection.EAST : EnumDirection.WEST;
                break;
            default:
                rotated = eDir;
                break;
        ...
        BlockStateDirection direction = BlockDirectional.FACING;
        IBlockData rotatedBlock = originalData.a(direction);
        rotatedBlock.set(direction, rotated);
        return rotatedBlock;
    }
