...
public abstract class AsyncTask extends BukkitRunnable {
    ...
    protected boolean checkFuel() throws ExecutionException, InterruptedException {
        ...
        final Block finalFuelHolder = fuelHolder;
        final double finalFuelBurnRate = fuelBurnRate;
        Bukkit.getScheduler().callSyncMethod(Movecraft.getInstance(), () -> {
            double processingFuelBurnRate = finalFuelBurnRate;
            InventoryHolder inventoryHolder = (InventoryHolder) finalFuelHolder.getState();
            for (ItemStack iStack : inventoryHolder.getInventory()) {
                ...
                //call event
                final FuelBurnEvent event = new FuelBurnEvent(craft, burningFuel, processingFuelBurnRate);
                Bukkit.getPluginManager().callEvent(event);
                if (event.getBurningFuel() != burningFuel)
                    burningFuel = event.getBurningFuel();
                if (event.getFuelBurnRate() != processingFuelBurnRate)
                    processingFuelBurnRate = event.getFuelBurnRate();
                if (burningFuel == 0.0) {
                    continue;
                }
                int amount = iStack.getAmount();
                int minAmount = 1;
                if (burningFuel < processingFuelBurnRate) {
                    minAmount = (int) processingFuelBurnRate;
                }
